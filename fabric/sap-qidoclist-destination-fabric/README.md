FABRIC SAP Queued IDoc List Destination Endpoint Quick Start
===========================================================
**Demonstrates the sap-qidoclist-destination component running in a Fabric camel runtime.**  
...
Summary: This quick start demonstrates how to configure and use the sap-qidoclist-destination component in a JBoss Fuse Fabric environment to send Intermediate Document (IDoc) Lists to SAP. This component sends IDoc document lists to SAP using the *Queued RFC* (qRFC) protocol.    
Target Product: Fuse  
...
* How to define, build and deploy a JBoss Fuse Fabric profile that configures a Fabric container to use the JBoss Fuse SAP Queued IDoc List Destination Camel component
...
1. Change your working directory to the quick start directory `sap-qidoclist-destination-fabric`.
...
* In the quick start directory run `mvn fabric8:deploy` to upload the quick start's profile (`sap-qidoclist-destination-fabric`) to the fabric container.
* Create a new child container and deploy the `sap-qidoclist-destination-fabric` profile in a single step, by entering the
 following command in the JBoss Fuse console:

        fabric:container-create-child --profile sap-qidoclist-destination-fabric root mychild
...
10. Copy the idoc list files (`src/data/idoclist?.xml`) in the project to the input directory (`instances/mychild/work/sap-qidoclist-destination-fabric/input`) of the quick start route.
