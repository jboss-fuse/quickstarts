FABRIC SAP IDoc List Destination Endpoint Quick Start
===========================================================
**Demonstrates the sap-idoclist-destination component running in a Fabric camel runtime.**  
...
Summary: This quick start demonstrates how to configure and use the sap-idoclist-destination component in a JBoss Fuse Fabric environment to send Intermediate Document (IDoc) Lists to SAP. This component sends IDoc document lists to SAP using the *Transactional RFC* (tRFC) protocol.    
Target Product: Fuse  
...
**NOTE:** This component does not guarantee the order of a series of document lists through its endpoints and the delivery and processing order of these document lists may differ on the receiving SAP system due to communication errors and resends of a document list. For guaranteed delivery order please see the JBoss Fuse SAP Queued IDoc List Destination Camel component.     

This quick start uses XML files containing serialized IDoc document lists to create Customer records in the Flight Data Application within SAP. These files are consumed by the quickstart's route and their contents are then converted to string message bodies. These messages are then routed to an `sap-idoclist-destination` endpoint which converts and sends them to SAP as `FLCUSTOMER_CREATEFROMDATA01` type IDoc document lists to create Customer records.  

In studying this quick start you will learn:

* How to define, build and deploy a JBoss Fuse Fabric profile that configures a Fabric container to use the JBoss Fuse SAP IDoc List Destination Camel component
* How to define a Camel route containing the JBoss Fuse SAP IDoc List Destination Camel component using the Blueprint XML syntax.
...
1. Change your working directory to the quick start directory `sap-idoclist-destination-fabric`.
...
* In the quick start directory run `mvn fabric8:deploy` to upload the quick start's profile (`sap-idoclist-destination-fabric`) to the fabric container.
* Create a new child container and deploy the `sap-idoclist-destination-fabric` profile in a single step, by entering the
 following command in the JBoss Fuse console:

        fabric:container-create-child --profile sap-idoclist-destination-fabric root mychild
...
10. Copy the idoc files (`src/data/idoc?.xml`) in the project to the input directory (`instances/mychild/work/sap-idoclist-destination-fabric/input`) of the quick start route.
...
3. Delete the quick start's profile by entering the following command at the JBoss Fuse console: 
		...
		fabric:profile-delete sap-idoclist-destination-fabric
